...
// D. LISTING
const listingSchemaDB = new mongoose.Schema({
...
listingSchemaDB.index({ title: 'text', description: 'text' }); 
const Listing = mongoose.models.Listing || mongoose.model('Listing', listingSchemaDB);

// E. STORY
const storySchemaDB = new mongoose.Schema({
    title: String, role: String, organization: String, excerpt: String, content: String, date: String, postedAt: { type: Date, default: Date.now }
});
const Story = mongoose.models.Story || mongoose.model('Story', storySchemaDB);
...
const startServer = async () => {
    try {
        ...
        // ACTUALIZARE PROFIL (CU TOT CU LISTINGS) - RESTAURAT
        app.put('/api/users/profile', async (req, res) => {
            try {
                ...
                // Actualizăm și anunțurile vechi
        ...
        // DELETE LISTING (RESTUARAT LOGICA ADMIN/OWNER)
