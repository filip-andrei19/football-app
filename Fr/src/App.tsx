...
import { Toaster } from 'react-hot-toast'; // <--- IMPORT NOU
...
import { NotFoundPage } from './components/NotFoundPage'; // <--- IMPORT NOU
import { AuthPage } from './components/AuthPage';

// --- HELPER BUTTON (Neschimbat) ---
...
type Section = 'home' | 'diaspora' | 'stars' | 'heroes' | 'collectors' | '404';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      try { setUser(JSON.parse(savedUser)); } catch (e) { console.error("Eroare user", e); }
    }
  }, []);
      ...
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 shadow-lg flex flex-col gap-2 animate-in slide-in-from-top-2">
            {navigation.map((item) => (
              <Button key={item.id} variant={activeSection === item.id ? 'secondary' : 'ghost'} onClick={() => { setActiveSection(item.id); setMobileMenuOpen(false); }} className="w-full justify-start gap-3 h-12 text-base">
                <item.icon className="h-5 w-5 text-gray-500" /> {item.label}
              </Button>
            ))}
            <div className="border-t border-gray-100 my-2"></div>
            <div className="flex flex-col gap-2">
              <div className="px-4 py-2 text-sm text-gray-500 font-semibold">Logat ca {user.name}</div>
