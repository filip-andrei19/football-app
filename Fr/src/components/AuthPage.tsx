...
export function AuthPage({ onLoginSuccess }: AuthPageProps) {
  ...
  // 1. MODIFICARE: Adăugat confirmPassword în state
  ...
  // Efect pentru rotirea imaginilor la fiecare 6 secunde
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. MODIFICARE: Validare confirmare parolă
    ...
    try {
      // Excludem confirmPassword din datele trimise la server
      const { confirmPassword, ...dataToSend } = formData;

      const response = await fetch(`https://football-backend-m2a4.onrender.com${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(isLogin ? formData : dataToSend) // Trimitem doar ce trebuie
      });
  ...
  return (
    <div className="min-h-screen w-full flex bg-white dark:bg-slate-900 overflow-hidden font-sans">
      ...
      {/* 3. MODIFICARE: Fundal mai viu cu gradient și elemente decorative */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 lg:p-12 relative bg-gradient-to-br from-blue-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 overflow-hidden">
        ...
        <div className="absolute top-6 left-6 md:hidden flex items-center gap-2 font-bold text-blue-900 dark:text-white z-20">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5 animate-in slide-in-from-top-2 fade-in duration-300">
                    ...
                    <div className="relative group">
                        ...
                        <input 
                            type="text" 
                            className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 bg-white/50 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 outline-none transition-all dark:bg-slate-900 dark:border-slate-600 dark:text-white font-medium"
                ...
                {/* 4. MODIFICARE: Câmp Confirmare Parolă (Doar la Register) */}
                {!isLogin && (
                <div className="space-y-1.5 animate-in slide-in-from-top-2 fade-in duration-300">
                    ...
                    <div className="relative group">
                        ...
                        <input 
                            type="password" 
                            className={`w-full pl-12 pr-4 py-3 rounded-xl border ${formData.confirmPassword && formData.password !== formData.confirmPassword ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white/50'} focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 outline-none transition-all dark:bg-slate-900 dark:border-slate-600 dark:text-white font-medium`}
                    ...
                    {formData.confirmPassword && formData.password !== formData.confirmPassword && (
                        <p className="text-xs text-red-500 ml-1 font-medium">Parolele nu coincid.</p>
                    )}
                </div>
                )}
                ...
                <button 
                    type="submit" 
                    disabled={loading || (!isLogin && formData.password !== formData.confirmPassword && formData.confirmPassword.length > 0)}
                    className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold text-lg rounded-xl shadow-lg shadow-blue-600/30 hover:shadow-blue-600/50 transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed mt-2"
                >
