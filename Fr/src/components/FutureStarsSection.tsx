...
export function FutureStarsSection() {
  ...
  useEffect(() => {
    const fetchYoungTalents = async () => {
      try {
        ...
        // LOGICA DE FILTRARE INIȚIALĂ (U21)
  ...
  return (
    <div className="relative min-h-[80vh] py-10 overflow-hidden bg-slate-50 dark:bg-slate-900">
      ...
      {/* --- ZONA DE BUTOANE (FILTRE) --- */}
      <section className="px-4 container mx-auto relative z-10 mb-10 flex justify-center">
          <div className="flex flex-wrap justify-center gap-2 bg-white/60 backdrop-blur-md p-2 rounded-2xl border border-amber-100 shadow-lg">
              {FILTERS.map((filter) => (
                  <button
                      key={filter}
                      ...
                      className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${
                          activeFilter === filter 
                          ? 'bg-amber-500 text-white shadow-md shadow-amber-500/30 transform scale-105' 
      ...
      <section className="px-4 container mx-auto relative z-10">
        {loading ? (
            ...
            <div className="text-center py-20 bg-white/50 backdrop-blur rounded-3xl border border-dashed border-gray-300 max-w-2xl mx-auto">
                <div className="text-6xl mb-4">🧤⚽</div>
                <p className="text-xl font-bold text-slate-700">Nu am găsit jucători pentru filtrul "{activeFilter}".</p>
                <p className="text-slate-500 mt-2">Încearcă altă poziție.</p>
                <button onClick={() => setActiveFilter("Toate")} className="mt-4 text-amber-600 font-bold hover:underline">Arată tot lotul</button>
            </div>
        ) : (
